...
export default function QuizPage() {
  ...
  const isLoading = selectedTopic
    ? topicQuestionsQuery.isLoading
    ...
    if (currentIndex < questions.length - 1) {
      ...
      const finalScore = finalAnswers.filter(
        (ans, i) =>
          ...
          ans !== undefined &&
          questions[i] &&
          Number(questions[i].correctAnswerIndex) === ans,
      ).length;
  ...
  const progressPercent =
    questions.length > 0
      ? ((currentIndex + (answers[currentIndex] !== undefined ? 1 : 0)) /
          questions.length) *
        100
      : 0;
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <p className="text-muted-foreground text-lg mb-4">
              Tiada soalan dijumpai untuk topik ini.
            </p>
